...
void Exti_Enable(uint8 LineNumber) {
    EXTI->IMR |= (0x01 << LineNumber); // Enable Interrupt mask register
    uint8 irqNumber = ExtiLineNumberNvicMap[LineNumber];
    NVIC->NVIC_ISER[irqNumber / 32] |= (0x01 << (irqNumber % 32)); // Map Exti to NVIC number to enable NVIC
}

void Exti_Disable(uint8 LineNumber) {
    EXTI->IMR &= ~(0x01 << LineNumber); // Disable Interrupt mask register
    uint8 irqNumber = ExtiLineNumberNvicMap[LineNumber];
    NVIC->NVIC_ICER[irqNumber / 32] |= (0x01 << (irqNumber % 32));  // Map Exti to NVIC number to disable NVIC
}
...
void EXTI9_5_IRQHandler(void) {
    if (EXTI->PR & (0x01 << 5)) { // read pending flag
        if (ExtiCallbacks[5]) {
            ExtiCallbacks[5]();
        }
